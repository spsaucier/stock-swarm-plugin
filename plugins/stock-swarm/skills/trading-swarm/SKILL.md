...
description: >-
  ...
  fundamentals analysts → bull/bear debate → research manager → trader → risk
  triangle debate → portfolio manager) without running the TradingAgents Python
...
3. Execute phases below, loading each persona skill **in character** (read its `SKILL.md` before that phase).
...
Cycle `researcher-bull` → `researcher-bear` for `2 × investment_rounds` turns.
...
Load only the relevant skill (e.g. `analyst-technical`) when the user wants one report, not the full swarm.
